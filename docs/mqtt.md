...
Публикуйте оба канала регулярно, включая FS24X без пламени, иначе через 180 секунд канал будет считаться устаревшим. fault=true означает неисправный канал; показания такого пакета не используются для оценки. Для HMP155 контракт всё равно требует числовые поля в диапазоне; при недоступном измерении шлюз может использовать последнее валидное значение с fault=true — оно не будет участвовать в fusion.

## Настройка симулятора

`SCENARIO`: normal / drying / fire / offline / fault / mixed.
`INTERVAL_SECONDS`: по умолчанию 60 (для быстрой демонстрации задайте `SENSOR_INTERVAL_SECONDS=5` в .env).
В Docker-демо симулятор раз в 10 секунд запрашивает у API список зарегистрированных датчиков и сам шлёт данные для **всех** участков, у которых есть датчики, в том числе созданных вами в интерфейсе. Три встроенных участка ведут себя по сценариям выше, остальные — по `DEFAULT_SCENARIO` (по умолчанию normal, со своим небольшим сдвигом температуры). Первое показание нового участка приходит в течение 10 секунд после регистрации датчика.

Без Docker (без `SIMULATOR_API_URL`) используется список `SIMULATOR_STATIONS`, пример:
