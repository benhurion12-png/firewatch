...
import {CoordinateFields} from '@/components/coordinate-fields';
...
export default function AreasPage(){
 ...
 {edit&&<FormModal title={edit==='new'?'Новый лесной участок':'Редактирование участка'} onClose={()=>setEdit(null)}><form onSubmit={save}><label>Название<input name="name" required minLength={2} maxLength={100} defaultValue={existing?.name} placeholder="Например, Бурабай · Северный лес" autoFocus/></label><label>Регион<input name="location" required minLength={2} maxLength={160} defaultValue={existing?.location} placeholder="Область или лесничество"/></label><CoordinateFields lat={existing?.lat} lng={existing?.lng}/><label>Площадь, га<input name="hectares" type="number" min={0} max={100000000} step="any" required defaultValue={existing?.hectares??0}/></label><label>Описание<textarea name="description" maxLength={2000} defaultValue={existing?.description} rows={3}/></label><ErrorBox message={formError}/><button className="button primary full" disabled={busy}>{busy?'Сохранение…':'Сохранить участок'}</button></form></FormModal>}
 </>;
}
